...
When an incident is detected by an error/monitoring system (Elasticsearch today; PagerDuty,
Sentry, etc. later), notifying the right people in Slack is currently a manual,
...
This tool removes that manual step. It receives incident events via webhook alert,
classifies them, resolves the relevant stakeholders from a versioned config, and posts a
structured alert to the appropriate Slack channel(s) in real time — so responders focus
on fixing the incident while all relevant parties are looped in automatically.

**Goal of the POC:** one source (Elasticsearch) → one Slack channel, with the architecture
...
- **Language/runtime:** Python 3.12 on **AWS Lambda**, fronted by **API Gateway** (HTTP API)
  to receive Elasticsearch webhook alerts.
...
- **Secrets:** Slack bot token + Elasticsearch webhook secret stored in AWS Secrets Manager;
  read at cold start, cached.

```
Elasticsearch alert ──> API Gateway ──> Lambda handler
                                            │
                                            ├─ verify signature
...
│   │   └── elasticsearch.py      # Elasticsearch payload → NormalizedIncident + sig verification
...
├── tests/
│   ├── fixtures/elasticsearch_*.json    # sample Elasticsearch webhook payloads
...
1. **Source adapter pattern** — `sources/base.py` defines a `SourceAdapter` protocol with
   `verify(request)` and `parse(payload) -> NormalizedIncident`. Elasticsearch is the only
   concrete adapter in the POC; PagerDuty/Sentry become new files implementing the
   same protocol. The handler selects the adapter by route/path (e.g. `/webhook/elasticsearch`),
...
4. **Severity classification** — for Elasticsearch, map its alert `priority`/`severity`
   metadata to the four internal severities in `routing.py`. Keep the mapping in config
   so it's tunable without redeploy.
...
3. Implement `sources/elasticsearch.py`: signature verification + payload→`NormalizedIncident`,
...
10. README: Slack app setup (bot token, scopes `chat:write`), Elasticsearch webhook alert
    config, deploy steps, and how to add a new source/channel.

## Critical files to create

- `src/incident_notifier/handler.py` — orchestration entrypoint.
- `src/incident_notifier/sources/elasticsearch.py` — first source adapter.
...
- **Local end-to-end:** run the handler locally (e.g. via a small `__main__` or SAM/
  `python -m`) and POST a saved Elasticsearch fixture; assert a message lands in a real
  test Slack channel (`#incidents-poc`) using a sandbox workspace token.
...
- **Deployed smoke test:** after `terraform apply`, trigger an Elasticsearch test alert
  (or curl the API Gateway URL with a signed fixture) and confirm the Slack post.
